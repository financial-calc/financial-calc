...
def test_discrete_effective_transaction_rate1(
    ...
    cr = capitalization_regime(term, day_basis)
    etr = cr.discrete_effective_transaction_rate(
        future_target_value, present_target_value, capitalization_regime="exp"
    )
    etr = round(etr, 4) * 100
...
def test_discrete_effective_transaction_rate2(
    ...
    cr = capitalization_regime(term, day_basis)
    etr = cr.discrete_effective_transaction_rate(
        future_target_value, present_target_value, capitalization_regime="lin"
    )
    etr = round(etr, 4) * 100
    assert etr == expected_result
...
@pytest.mark.parametrize(
    "term, day_basis, target_value, fee, expected_result",
    [(3, 1, 100, 0.016263, 105.00), (5, 1, 20000, 0.09, 31366.24)],
)
...
@pytest.mark.parametrize(
    "term, day_basis, target_value, fee, expected_result",
    [(3, 1, 105, 0.016263, 100.00), (5, 1, 31366.24, 0.09, 20000.00)],
)
...
def test_continuous_effective_transaction_rate(
    ...
    cr = capitalization_regime(term, day_basis)
    etr = cr.continuous_effective_transaction_rate(
...
def test_deferred_uniform_payments_series(
    term, day_basis, fee, present_target_value, expected_result
):
...
@pytest.mark.parametrize(
    ...
    [(180, 30, 0.05, 3000.00, 15227.08), (12, 1, 0.02, 100.00, 1057.53)],
)
def test_present_value_deferred_uniform_series(
    term, day_basis, fee, payments, expected_result
):
    ups = uniform_payments_series(term, day_basis)
...
def test_advance_uniform_payments_series(
    term, day_basis, fee, present_target_value, expected_result
):
    ups = uniform_payments_series(term, day_basis)
...
@pytest.mark.parametrize(
    "term, day_basis, fee, payments, expected_result",
    [(120, 30, 0.07, 200.00, 724.86), (12, 1, 0.02, 100.00, 1078.68)],
)
def test_present_value_advance_uniform_series(
    term, day_basis, fee, payments, expected_result
):
    ups = uniform_payments_series(term, day_basis)
...
def test_future_value_uniform_payments_series1(
    term, day_basis, fee, payments, future_target_value, expected_result
):
    ups = uniform_payments_series(term, day_basis)
...
def test_future_value_uniform_payments_series2(
    term, day_basis, fee, payments, future_target_value, expected_result
):
    ups = uniform_payments_series(term, day_basis)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([-10000.00, 2500.00, 2500.00, 2500.00, 3000.00, 3000.00], 10.48),
    ],
)
