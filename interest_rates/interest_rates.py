...
class equivalent_rates:
    ...
    def calculate_equivalent_rates(self, capitalization_regime='exp'):
        ...
        if capitalization_regime == 'lin':
            required_rate = self.rate_to_convert * (self.term_to_convert / self.day_basis_to_convert) / (
                self.required_term / self.required_day_basis)
            return required_rate

        elif capitalization_regime == 'exp':
            required_rate = ((1 + self.rate_to_convert) ** ((self.term_to_convert / self.day_basis_to_convert) / (
                self.required_term / self.required_day_basis))) - 1
            return required_rate
